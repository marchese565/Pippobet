...
# =============================================================================
# LOGICA - Quote doppie (1X, X2, 12) da quote 1, X, 2
# =============================================================================


def quote_doppie_da_probabilita(
    probabilita: list[float], margine: float = 0.05, tolleranza: float = TOLERANZA_PROBABILITA
) -> dict[str, float]:
    """
    Calcola le quote per le doppie chance (1X, X2, 12) a partire dalle probabilità 1, X, 2.
    - 1X: vittoria casa o pareggio  (P = p1 + pX)
    - X2: pareggio o vittoria trasferta (P = pX + p2)
    - 12: vittoria casa o trasferta, no pareggio (P = p1 + p2)
    """
    if not probabilita_valide(probabilita, tolleranza) or len(probabilita) != 3:
        raise ValueError("Serve una lista di 3 probabilità valide (1, X, 2)")
    p1, px, p2 = probabilita
    prob_1x = p1 + px
    prob_x2 = px + p2
    prob_12 = p1 + p2
    q1x = quota_bookmaker(quota_giusta(prob_1x), margine)
    qx2 = quota_bookmaker(quota_giusta(prob_x2), margine)
    q12 = quota_bookmaker(quota_giusta(prob_12), margine)
    return {"1X": q1x, "X2": qx2, "12": q12}


def simula_esito(probabilita: list[float], seed: int | None = None) -> int:
    """
...
def _poisson_p(k: int, lam: float) -> float:
    """Probabilità di esattamente k gol (Poisson)."""
    import math
...
def _poisson_cdf(k: int, lam: float) -> float:
    """P(X <= k) per Poisson."""
    return sum(_poisson_p(i, lam) for i in range(k + 1))


def quote_over_under(
    forza_casa: float, forza_trasferta: float, margine: float = 0.05
) -> dict[str, float]:
    """
    Calcola quote Over/Under 1.5, 2.5, 3.5.
    Usa modello Poisson: lambda = gol attesi in base alle forze.
    """
    lam = 1.8 + 0.012 * (forza_casa + forza_trasferta)
    lam = max(1.5, min(3.5, lam))
    out = {}
    for soglia in [1, 2, 3]:
        p_under = _poisson_cdf(soglia, lam)
        p_over = 1 - p_under
        out[f"Over{soglia}.5"] = round(quota_bookmaker(quota_giusta(p_over), margine), 2)
        out[f"Under{soglia}.5"] = round(quota_bookmaker(quota_giusta(p_under), margine), 2)
    return out


def simula_gol(forza_casa: float, forza_trasferta: float, seed: int | None = None) -> int:
    """Simula il numero di gol totali (Poisson)."""
    if seed is not None:
        random.seed(seed)
    lam = 1.8 + 0.012 * (forza_casa + forza_trasferta)
    lam = max(1.5, min(3.5, lam))
    ...
    cum = 0
    for k in range(15):
        cum += _poisson_p(k, lam)
        if r < cum:
            return k
    return 10


def scommessa_ou_vincente(tipo: str, gol_totali: int) -> bool:
    """Verifica se Over/Under vince."""
    if tipo.startswith("Over"):
        soglia = int(tipo.replace("Over", "").replace(".5", ""))
        return gol_totali > soglia
    if tipo.startswith("Under"):
        soglia = int(tipo.replace("Under", "").replace(".5", ""))
        return gol_totali < soglia
    return False


def scommessa_vincente(tipo_scommessa: str, esito: int) -> bool:
    """Verifica se la scommessa vince dato l'esito (0=1, 1=X, 2=2)."""
    match tipo_scommessa.upper():
        case "1":
            return esito == 0
        case "X":
            return esito == 1
...
def stampa_prospetto_quote(
    ...
    labels = esiti if esiti else [str(i) for i in range(1, len(probabilita) + 1)]
    larghezza = 12
    sep = "+" + "-" * (larghezza + 2) * 4 + "+"
    ...
    print(sep)
    for lab, p, qf, qb in zip(labels, probabilita, quote_fair, quote_book):
        print(riga.format(lab, f"{p:.2%}", f"{qf:.4f}", f"{qb:.4f}"))
    print(sep)
